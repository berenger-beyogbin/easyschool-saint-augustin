...
- [x] **D6** (ciblé) — Correctif précis matching le constat de l'audit ("certaines vues statistiques peuvent être construites même si leur onglet n'est pas affiché") : `views/statistiques_view.py` et `views/versements_view.py` instanciaient `StatCantineView`/`StatTransportView`/`MontantCantineView`/`MontantTransportView` **inconditionnellement**, même quand `ENABLE_CANTINE`/`ENABLE_TRANSPORT` sont à `False` (seul l'ajout de l'onglet était conditionnel). Ces vues ne sont désormais construites que si le module est actif.
  Vérifié réellement en Qt headless (`QT_QPA_PLATFORM=offscreen`) : avec les flags à `False` (défaut CJGA), les vues sont bien `None`, les onglets restants fonctionnent ; avec les flags à `True`, les vues se construisent et le changement d'onglet vers Transport/Cantine fonctionne sans exception.
  **Suppression complète du code mort (modèles, autres vues) non faite** — l'audit lui-même recommande une architecture à bascule plutôt qu'une suppression, et CJGA pourrait réactiver ces modules ; périmètre volontairement limité à l'inefficacité concrète relevée.
