...
class NationaliteView(QWidget):
    """Écran — Gestion des nationalités."""
    ...
    def init_ui(self):
        ...
        titre = QLabel("Gestion des Nationalités")
        ...
        lbl = QLabel("Nationalité * :")
        lbl.setStyleSheet(
            f"font-size: 12px; font-weight: 600; color: {COLORS['text_soft']};"
            "background-color: transparent;"
        )
        self.txt_nat = QLineEdit()
        self.txt_nat.setPlaceholderText("Ex : IVOIRIENNE, FRANÇAISE")
        self.txt_nat.setStyleSheet(INPUT_STYLE)
        self.txt_nat.setMaximumWidth(280)
        ...
        self.btn_valider.clicked.connect(self.save_nationalite)
        ...
        form_layout.addWidget(self.txt_nat)
        ...
        self.table.setHorizontalHeaderLabels(["Nationalité", "Supprimer"])
    ...
    def load_data(self):
        ...
        for i, item in enumerate(NationaliteService.get_all()):
            self.table.insertRow(i)
            self.table.setItem(i, 0, QTableWidgetItem(item.Nationalite))
            btn_suppr = QPushButton("Supprimer")
            configure_table_action_button(btn_suppr, "danger")
            btn_suppr.clicked.connect(
                lambda checked=False, id_n=item.IDTNationalite, lib=item.Nationalite:
                self.supprimer_nationalite(id_n, lib)
            )
            self.table.setCellWidget(i, 1, make_table_action_container(btn_suppr))
            self.table.setRowHeight(i, 44)

    def save_nationalite(self):
        lib = self.txt_nat.text().strip().upper()
        if not lib:
            QMessageBox.warning(self, "Champ obligatoire", "Veuillez saisir le libellé de la nationalité !")
            return
        reussite, msg = NationaliteService.add_nationalite(lib)
        if reussite:
    ...
    def supprimer_nationalite(self, id_nat, libelle):
        reponse = QMessageBox.question(
            self, "Confirmer la suppression",
            f"Voulez-vous vraiment retirer la nationalité '{libelle}' ?",
            QMessageBox.Yes | QMessageBox.No
        )
        if reponse == QMessageBox.Yes:
            reussite, msg = NationaliteService.delete_nationalite(id_nat)
            if reussite:
