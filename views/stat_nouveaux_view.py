...
from utils.list_printer import InscritsListPrinter


class StatNouveauxView(QWidget):
    ...
    def init_ui(self):
        ...
        titre = QLabel("Liste des Nouveaux Élèves")
        titre.setStyleSheet(PAGE_TITLE_STYLE)
        layout.addWidget(titre)

        lbl_section = QLabel("État des nouveaux inscrits")
        ...
        lbl_util = QLabel("Utilisateur :")
        lbl_util.setStyleSheet(lbl_style)
        self.cmb_utilisateur = QComboBox()
        self.cmb_utilisateur.setStyleSheet(COMBO_STYLE)
        self.cmb_utilisateur.setMinimumWidth(130)
        ...
        self.cmb_niveau.setMinimumWidth(130)
        ...
        self.cmb_classe.setMinimumWidth(130)
        ...
        layout_filtres.addWidget(lbl_util)
        layout_filtres.addWidget(self.cmb_utilisateur)
        ...
        self.table.setHorizontalHeaderLabels([
            "N°", "Inscrit Le", "Matricule", "DatNaiss", "Noms", "Classe"
        ])
    ...
    def load_filters(self):
        self.cmb_utilisateur.clear()
        self.cmb_utilisateur.addItem("-- Tous --", None)
        logins = StatistiquesService.get_utilisateurs_inscriptions()
        for login in logins:
            self.cmb_utilisateur.addItem(login, login)

        self.cmb_niveau.blockSignals(True)
        self.cmb_niveau.clear()
        self.cmb_niveau.addItem("-- Tous --", None)
    ...
    def load_classes_filter(self, id_niveau):
        ...
        self.cmb_classe.addItem("-- Toutes --", None)
    ...
    def refresh_data(self):
        login = self.cmb_utilisateur.currentData()
        id_niveau = self.cmb_niveau.currentData()
        id_classe = self.cmb_classe.currentData()

        data = StatistiquesService.get_nouveaux_inscrits(login, id_niveau, id_classe)

        self.table.setRowCount(0)
        self.table.setRowCount(len(data))

        for idx, item in enumerate(data):
            date_insc = item["DateInscription"].strftime("%d/%m/%Y") if item["DateInscription"] else ""
            matr = item["Matricule"] or ""
            dat_naiss = item["DateNaissance"].strftime("%d/%m/%Y") if item["DateNaissance"] else ""
            ...
            self.table.setItem(idx, 1, it_date)
            self.table.setItem(idx, 2, it_matr)
            self.table.setItem(idx, 3, it_naiss)
            self.table.setItem(idx, 4, it_noms)
            self.table.setItem(idx, 5, it_classe)
    ...
    def imprimer_clic(self):
        ...
        InscritsListPrinter.print_list(self, data, filtre)
